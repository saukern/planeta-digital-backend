...
export const subirArchivoPersonal = async (req, res) => {
	try {
		...
		// Transaccion en la base de datos para guardar la herencia y asociarla al usuario
		const resultado = await prisma.$transaction(async (tx) => {
			// Creamos registro en la tabla de archivos
			...
			// Creamos el  registro en la tabla especializada correspondiente
			if (tipo === 'libro') {
			...
			// Creamos registro en la tabla de 'progreso_usuario' para asociarlo a este usuario
...
export const editarArchivo = async (req, res) => {
	try {
		...
		// Actualizamos los datos en la tabla 'archivos'
...
export const eliminarDeBiblioteca = async (req, res) => {
	try {
		...
		//Eliminamos del progreso de la biblioteca del usuario
		await prisma.progresoUsuario.delete({
			where: { id: progresoId }
		});

		// Buscamos si algun otro usuario en la plataforma tiene este archivo en su biblioteca 
		...
		// Si nadie mas lo tiene lo eliminamos de todos lados 
		if (otrosProgresos === 0) {
			...
			if (archivo) {
				...
				// Borramos el archivo y la portada de la nube de supebase 
...
export const importarDesdeGutendex = async (req, res) => {
	try {
		...
		// Subimos el archivo EPUB a Supabase
		let urlNube;
		try {
		...
		// Subimos la portada (opcional) a Supabase
		let urlPortada = null;
		if (rawImageUrl) {
			try {
		...
		const resultado = await prisma.$transaction(async (tx) => {
			...
			// Creamos el progreso asociado al usuario
			const nuevoProgreso = await tx.progresoUsuario.create({
				data: {
					usuario_id: req.usuario.id,
					archivo_id: nuevoArchivo.id,
